...
var updateAirTraffic = function(){
  if((dispatch.counter < 100) && queue[queue.length-1]){ //maintains maximum four phantoms
    dispatch.counter++;
...
var phantomFetch = function(kiwi){
  if(kiwi.user !== 'facebook:10152230734793224'){
  dispatch.counter--;
  return;
  } //filter for only one user's kiwis. For debugging. 
