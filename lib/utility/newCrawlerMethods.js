...
var updateAirTraffic = function(){
  if((dispatch.counter < exports.phantoms) && queue[queue.length-1]){ //maintains maximum four phantoms
    dispatch.counter++;
    console.log('Dispatched phantom ' + dispatch.counter + '. Now fetching kiwi at ' + queue[queue.length-1].destination);
...
exports.populate = function(item){
  ...
  console.log('Spinning up worker...')
    ...
    for(var user in dataSet){
      dataSet[user].notified = item[user].settings.notified;
      dataSet[user].email = item[user].settings.email;
      dataSet[user].name = item[user].settings.name;
...
var shutDown = function(passedIn, phantom){
  ...
  if(dispatch.counter === 0){
    setTimeout(function(){
      dataSet = {};
      exports.working = false;
      console.log('Crawl complete. Worker idling.')
    }, 5000); //allow phantoms time to exit and end processes. reduces risk of memory leak.
  }
  phantom.exit(); 
}
