describe('value parser', function () {
  it('exists', function () {
    expect(NumberParser).to.exist;
    expect(NumberParser).to.be.a('function');
  });

  it('parses the value with a naive algorithm', function () {
    var original = "$585.64";
    var rest = ["$585.64$581.35", "$585.66$582.25", "$586.03$583.38"];
    var parser = new NumberParser(original, rest);
  ...
  it('cleans numbers', function () {
    var parser = new NumberParser("50", rest);
    var rest = ["501261645More "];
    expect(parser._cleanNumber(rest[0])).to.equal("501261645");
  });
  ...
  it('parses when there are additional crap values', function () {
    ...
    var parser = new NumberParser(original, rest);
    expect(parser.parse()).to.eql(["7,235,750,132"]);
  });
